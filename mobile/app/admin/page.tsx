...
import { getAdminToken, loginAdmin } from "@/lib/api";
    ...
    <div className="app-root" style={{ paddingBottom: 0 }}>
      <main className="app-main" style={{ alignSelf: "center", width: "100%", maxWidth: 420, paddingTop: 48 }}>
        <div className="center" style={{ marginBottom: 24 }}>
          ...
          <h1 style={{ margin: "8px 0 2px", fontSize: 22 }}>Panel de administración</h1>
          <p className="muted" style={{ fontSize: 14, margin: 0 }}>Gestión del programa y membresías</p>
        </div>
        <form onSubmit={onSubmit}>
          <div className="field">
            <label htmlFor="email">Correo</label>
            <input id="email" type="email" autoComplete="username" value={email}
              onChange={(e) => setEmail(e.target.value)} placeholder="tenantadmin@burger.dev" required />
          </div>
          <div className="field">
        ...
        <p className="muted center" style={{ fontSize: 12, marginTop: 16 }}>
          Demo: <code>tenantadmin@burger.dev</code> (admin) · <code>admin@loyalty.dev</code> (super)
        </p>
      </main>
    </div>
  );
